...
# Ensure the parent ScrapCompiler package is importable
...
def _extract_module_defs(source: str) -> dict:
    ...
    while i < len(lines):
        ...
        if line.startswith('module '):
            ...
            # Scan forward for inputs/outputs blocks
            j = i + 1
            while j < len(lines):
...
def _parse_block(lines: list, start: int, block_name: str):
    ...
    # Skip the block header (e.g., 'inputs' or 'inputs {')
    block_line = lines[j].strip()
...
def _parse_field(line: str) -> dict:
    ...
    # Handle length annotations like dynamic[a] b
    if '[' in line:
    ...
    # Find the field name (last token after removing annotations)
    name_parts = []
    type_parts = []
    for part in parts:
        if part in ('buffered', 'dynamic', 'bit', 'bool', 'int'):
...
def _build_variable_info(gates) -> dict:
    ...
    for var, var_gates in variable_gates.items():
        ...
        bit_str = ''.join(
            '1' if g.type == 'OR' and not g.inputs else
            '0' if g.type == 'NOT' and not g.inputs else
            '?'
            for g in bits
        )
        value = None
...
def _build_module_info(modules: dict, compiler) -> dict:
    ...
    for name in list(modules.keys()) + ['IntInput', 'IntDisplay', 'Lamp', 'Switch', 'Button']:
        if name in result:
            continue
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print(json.dumps({'error': 'Usage: analyze.py <file.logic>'}))
        sys.exit(0)
    ...
    try:
        ...
        print(json.dumps({'error': str(e), 'variables': {}, 'modules': {}}))
    sys.exit(0)
